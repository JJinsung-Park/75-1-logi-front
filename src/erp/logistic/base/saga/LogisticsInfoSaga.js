...
import {addItemDetailList} from "../action/LogisticsInfoAction";
...
function* addDetailCodeInfo(action) {
    ...
    try {
        yield put({ type: types.DETAIL_CODE_LIST,
                    mode: 'add',
...
const saveItemDetail=createRequestSaga(types.SAVE_ITEM_DETAIL_LIST, api.saveItemBatchList);
 ...
 export function* addItemDetail(action){
     try {
         yield put({ type: types.ADD_ITEM_DETAIL_LIST,
             mode: 'add',
             itemTo: action.payload });
     } catch (error) {
         alert(error);
     }
 };
